import {IProfilerListRow, IProfilerListViewModel, IRequestProfile, ProfilerListTab} from "../types/debugToolbar.types";
...
export function buildProfilerListViewModel(
    ...
    if (ip) {
        filtered = filtered.filter((p: IRequestProfile): boolean => (p.request?.ip ?? "")
            .toLowerCase()
            .includes(ip.toLowerCase()));
    }
    if (token) {
        filtered = filtered.filter((p: IRequestProfile): boolean => p.token
            .toLowerCase()
            .startsWith(token.toLowerCase()));
    }
    if (search) {
        filtered = filtered.filter((p: IRequestProfile): boolean => p.url
            .toLowerCase()
    ...
    if (status) {
        ...
        if (code >= 100) {
            ...
            filtered = filtered.filter((p: IRequestProfile): boolean => p.statusCode >= prefix
                && p.statusCode < prefix + 100);
        }
    }
    if (from) {
        const ts: number = new Date(from).getTime();
        if (!isNaN(ts)) {
            filtered = filtered.filter((p: IRequestProfile): boolean => p.timestamp >= ts);
        }
    }
    if (until) {
    ...
    const rows: IProfilerListRow[] = filtered.map((p: IRequestProfile) => ({
        token: p.token,
    ...
    return {
        ...
        tabs: {
            requests: {
                href: tabHref("requests", search, method, status, ip, token, from, until, limit),
                active: tab === "requests"
            },
            commands: {
                href: tabHref("commands", search, method, status, ip, token, from, until, limit),
                active: tab === "commands"
            },
        },
    };
